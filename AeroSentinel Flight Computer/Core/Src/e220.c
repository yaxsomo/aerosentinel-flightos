...
int8_t E220_set_broadcast_address(struct LoRa_Handler *LoRa)
{

	//uint8_t data_H=E220_read_register(LoRa,REG_ADD_H);
	//uint8_t data_L=E220_read_register(LoRa,REG_ADD_L);
    E220_write_register(LoRa, REG_ADD_H, 0xFF);
    E220_write_register(LoRa, REG_ADD_L, 0xFF);
	if((uint8_t)E220_read_register(LoRa,REG_ADD_H)==(0xFF) && (uint8_t)E220_read_register(LoRa,REG_ADD_L)==(0xFF)) return 1;
	else return -1;
}


int8_t E220_set_channel(struct LoRa_Handler *LoRa, uint8_t channel)
{

	uint8_t data=E220_read_register(LoRa,REG2);
    E220_write_register(LoRa, REG2, data | channel);
	if((uint8_t)E220_read_register(LoRa,REG2)==(data | channel)) return 1;
	else return -1;
}
